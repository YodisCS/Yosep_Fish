...
function uploadGambar($file_key, $gambar_lama = '')
{
  ...
  if (!in_array($ext, $ext_allowed)) {
    echo json_encode(['success' => false, 'msg' => 'Format gambar tidak valid (jpg/png/webp/gif)']);
    exit;
  }
  if ($_FILES[$file_key]['size'] > 3 * 1024 * 1024) {
    echo json_encode(['success' => false, 'msg' => 'Ukuran gambar maksimal 3MB']);
    exit;
  }
...
if (isset($_GET['ajax'])) {
  ...
  if ($action === 'get_dashboard') {
    ...
    $params = [];
    if ($status) {
      $sql .= " AND p.status_pembelian = :status";
      $params[':status'] = $status;
    }
    if ($search) {
      $sql .= " AND (p.invoice LIKE :s1 OR p.nama_pemesan LIKE :s2 OR p.nomor_hp LIKE :s3)";
      $params[':s1'] = "%$search%";
      $params[':s2'] = "%$search%";
      $params[':s3'] = "%$search%";
    }
    ...
    $params = [];
    if ($search) {
      $sql .= " AND (nama LIKE :s1 OR warna LIKE :s2 OR kode_produk LIKE :s3)";
      $params[':s1'] = "%$search%";
      $params[':s2'] = "%$search%";
      $params[':s3'] = "%$search%";
    }
...
<body>
      ...
      <div class="content">
        ...
        <div id="pageDashboard" class="page active">
          <div class="stats-grid" id="statsGrid">
            <div class="stat-card blue">
              ...
              <div class="stat-value" id="s_revenue">…</div>
              <div class="stat-meta">Pesanan selesai</div>
            </div>
            <div class="stat-card green">
              <div class="stat-icon green">📦</div>
              <div class="stat-label">Total Pesanan</div>
              <div class="stat-value" id="s_pesanan">…</div>
              <div class="stat-meta" id="s_selesai_meta">…</div>
            </div>
            <div class="stat-card orange">
              <div class="stat-icon orange">🐟</div>
              <div class="stat-label">Produk Aktif</div>
              <div class="stat-value" id="s_produk">…</div>
              <div class="stat-meta" id="s_habis_meta">…</div>
            </div>
            <div class="stat-card yellow">
              <div class="stat-icon yellow">⏳</div>
              <div class="stat-label">Menunggu Bayar</div>
              <div class="stat-value" id="s_menunggu">…</div>
              <div class="stat-meta">Perlu konfirmasi</div>
            </div>
          </div>
              ...
              <div class="table-wrap">
                <table>
                  ...
                  <tbody id="recentOrdersTbl">
                    <tr>
                      <td colspan="4" style="text-align:center;color:var(--text3);padding:20px">Memuat…</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="card">
              ...
              <div class="table-wrap">
                <table>
                  ...
                  <tbody id="topProdukTbl">
                    <tr>
                      <td colspan="3" style="text-align:center;color:var(--text3);padding:20px">Memuat…</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          <div class="card">
            <div class="table-wrap">
              <table>
                ...
                <tbody id="pesananTbl">
                  <tr>
                    <td colspan="9" style="text-align:center;color:var(--text3);padding:30px">Memuat data…</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div id="pageLaporan" class="page">
          ...
          <div class="report-kpi" id="reportKpi">
            <div class="kpi-box">
              <div class="kpi-val" id="kpi_rev" style="color:var(--cyan)">…</div>
              <div class="kpi-lbl">Total Revenue</div>
            </div>
            <div class="kpi-box">
              <div class="kpi-val" id="kpi_pesanan" style="color:var(--green)">…</div>
              <div class="kpi-lbl">Pesanan Masuk</div>
            </div>
            <div class="kpi-box">
              <div class="kpi-val" id="kpi_item" style="color:var(--yellow)">…</div>
              <div class="kpi-lbl">Ikan Terjual</div>
            </div>
            <div class="kpi-box">
              <div class="kpi-val" id="kpi_avg" style="color:var(--orange)">…</div>
          ...
          <div class="grid-65" style="margin-bottom:16px">
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="chart-wrap" style="height:220px"><canvas id="chartMetode"></canvas></div>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div class="table-wrap">
              <table>
                ...
                <tbody id="terlarisLapTbl">
                  <tr>
                    <td colspan="5" style="text-align:center;color:var(--text3);padding:20px">Memuat…</td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  <script>
    ...
    const fmtK = n => {
      n = parseInt(n);
      return n >= 1000000 ? (n / 1000000).toFixed(1) + 'jt' : n >= 1000 ? (n / 1000).toFixed(0) + 'rb' : n;
    };
      ...
      fetch('?ajax=get_dashboard')
        ...
        .then(res => {
          ...
          if (s.menunggu_bayar > 0) {
            ...
            b.textContent = s.menunggu_bayar;
          }
    ...
    function viewInvoice(invoice) {
      fetch('?ajax=get_order_detail&invoice=' + encodeURIComponent(invoice))
        ...
        .then(res => {
          ...
          const jam = new Date(p.created_at).toLocaleTimeString('id-ID', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit'
          });
        ...
        .then(rows => {
          ...
          grid.innerHTML = rows.map(p => {
            const statusMap = {
              ready: 'pill-green',
              habis: 'pill-red',
              pre_order: 'pill-yellow',
              tidak_aktif: 'pill-gray'
            };
            const statusLbl = {
              ready: 'Ready',
              habis: 'Habis',
