/* ==========================================
   YOSEP FISH — SCRIPT.JS
   Handles: Landing Page + Auth (Login/Register)
   ========================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Tab switching (login.php / register.php share same design) ──
  ...
  if (registerForm) {
    ...
    registerForm.addEventListener('submit', e => {
      ...
      if (!noHp?.value.trim() || !/^\d{8,13}$/.test(noHp.value.replace(/\s/g, ''))) {
  ...
  function showError(input, message) {
    ...
    const group = input.closest('.form-group') || input.closest('.input-wrapper');
